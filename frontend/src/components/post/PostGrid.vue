...
<template>
  <div class="grid grid-cols-5 gap-5">
    <div
      ...
      :class="{
        'flex items-center justify-center h-[200px] border border-gray-100':
          isTextCard(post),
      }"
      @click="emit('open', post.id)"
    >
      <template v-if="isTextCard(post)">
        <div class="text-center font-bold text-lg whitespace-pre-line p-4">
          {{ post.title }}
        </div>
      </template>
      <template v-else>
        ...
        >
          ...
          />
          <div
            class="absolute inset-0 bg-gradient-to-b from-black/5 to-black/25 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
          ></div>
        </div>
        <div class="p-3">
          <div class="text-sm font-medium leading-[1.4] line-clamp-2">
            {{ post.title }}
          </div>
          <div
            v-if="post.likeCount"
            class="flex justify-end items-center text-xs text-gray-500 mt-2"
          >
    ...
    <div
      v-if="posts.length === 0"
      class="col-span-5 text-center text-gray-400 py-20"
    >
      {{ emptyText }}
    </div>
  </div>
