...
<template>
  <div class="fixed inset-0 bg-black/30 z-[100] flex justify-center items-center" @click.self="handleClose">
    <div class="relative bg-white rounded-2xl p-8 w-[640px] max-w-[92%] max-h-[90vh] overflow-y-auto shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
      ...
      <!-- 错误提示 -->
      <div v-if="errorMsg" class="mb-4 text-sm text-red-500 text-center bg-red-50 rounded-lg py-2 px-3">
        {{ errorMsg }}
      </div>
      ...
      <!-- 图片上传 -->
      <div class="mb-5">
        <label class="block text-sm font-medium text-gray-600 mb-2">图片 <span class="text-xs text-gray-400">(最多9张)</span></label>
