...
- **URL**: `GET /api/post/text-image/generate`
- **权限**: 公开
- **说明**: 根据输入文本生成一张 2:3 比例的 PNG 配图，适用于笔记封面等场景。
- **查询参数**:

| 参数   | 类型     | 必填 | 说明        |
| ------ | ------ | -- | --------- |
| text   | string | 是  | 文本内容，最多20字 |

- **成功响应** (200): 返回 `Content-Type: image/png` 的二进制图片流
- **失败场景**:
  - text 为空 → `code: 5002`
  - text 超过20字 → `code: 5001`
